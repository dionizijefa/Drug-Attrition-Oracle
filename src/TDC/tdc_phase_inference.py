...
@click.option('-out_prefix', default='')
@click.option('-batch_size', default=32)
@click.option('-seed', default=0)
def main(train_data, test_data, withdrawn_col, out_prefix, batch_size, seed):
    ...
    for subdir in admes.iterdir():
        ...
        with torch.no_grad():
            ...
            for i in tqdm(data_loader):
                train_outputs.append(model.forward(i.x.cuda(), i.edge_index.cuda(), i.batch.cuda()))
            ...
            for i in tqdm(test_loader):
                test_outputs.append(model.forward(i.x.cuda(), i.edge_index.cuda(), i.batch.cuda()))
    ...
    train_results_df.to_csv(root / 'data/processing_pipeline/TDC_predictions/{}_train_subtasks_predictions.csv'.format(out_prefix))
    ...
    test_results_df.to_csv(root / 'data/processing_pipeline/TDC_predictions/{}_test_subtasks_predictions.csv'.format(out_prefix))
