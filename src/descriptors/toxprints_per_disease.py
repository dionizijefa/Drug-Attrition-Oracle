...
def main(train_data, test_data, withdrawn_col, seed):
    train = pd.read_csv(root / 'data/processing_pipeline/{}'.format(train_data), index_col=0)[['chembl_id', withdrawn_col]]
    train = train.sample(frac=1, random_state=seed)  # shuffle
    test = pd.read_csv(root / 'data/processing_pipeline/{}'.format(test_data), index_col=0)[['chembl_id', withdrawn_col]]
